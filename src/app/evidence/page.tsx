...
// id: the part after watch?v= in the YouTube URL
...
export default function EvidencePage() {
  ...
  const pendingVideos = videos.filter((v) => v.id === "");
      ...
      <section className="mb-12">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {[
            {
              title: "多目的広場（駐車場化前）",
              desc: "天然芝のサッカー場として利用されていた頃の状態。（写真追加予定）",
            },
            {
              title: "多目的広場（2026年8月 駐車場化後）",
              desc: "プール利用者向け臨時駐車場として使用されている状態。（写真追加予定）",
            },
            {
              title: "フェンスへの張り紙",
              desc: "利用者への通知として掲示されたもの。説明会等は行われていない。（写真追加予定）",
            },
            {
              title: "利用スケジュール掲示",
              desc: "土日がほぼ全日利用不可となったスケジュール掲示。（写真追加予定）",
            },
          ].map((item) => (
            <div
              key={item.title}
